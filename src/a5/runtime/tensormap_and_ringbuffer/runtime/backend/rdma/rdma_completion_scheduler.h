...
inline CompletionPollResult poll_rdma_hns1825_cqe_record(uint64_t cqe_addr, uint32_t expected_owner) {
    if (cqe_addr == 0 || expected_owner > 1U) {
    ...
    const uint32_t owner = (owner_id_qpn >> kOwnerShift) & 1U;
    if (owner != expected_owner) {
        return {CompletionPollState::PENDING, SIMPLER_ERROR_NONE};
    }

    const uint32_t cqe_type = (op_sr_wqebb >> kCqeOpcodeShift) & kCqeOpcodeMask;
    if (cqe_type == kCqeOptypeInvalid) {
        return {CompletionPollState::PENDING, SIMPLER_ERROR_NONE};
    }
...
inline void
log_rdma_hns1825_cqe_record_snapshot(uint64_t cqe_addr, uint32_t expected_owner, int32_t entry_idx, int32_t cond_idx) {
    if (cqe_addr == 0) {
        return;
    }
    auto *cqe = reinterpret_cast<volatile Hns1825Cqe *>(static_cast<uintptr_t>(cqe_addr));
    ...
    const uint32_t cqe_type = (op_sr_wqebb >> kCqeOpcodeShift) & kCqeOpcodeMask;
    LOG_INFO(
        "[ASYNC_WAIT RDMA entry=%d cond=%d] hns1825_cqe addr=0x%llx owner=%u expected=%u type=0x%x "
        "owner_id_qpn=0x%x op_sr_wqebb=0x%x syndrome=0x%x",
        entry_idx, cond_idx, static_cast<unsigned long long>(cqe_addr), owner, expected_owner, cqe_type, owner_id_qpn,
        op_sr_wqebb, static_cast<unsigned>(__atomic_load_n(&cqe->syndrome, __ATOMIC_ACQUIRE))
    );
}
