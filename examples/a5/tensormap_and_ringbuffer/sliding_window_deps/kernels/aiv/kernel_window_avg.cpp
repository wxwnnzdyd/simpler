...
static __aicore__ void window_mean_impl(__gm__ float *const *srcs, __gm__ float *out) {
    ...
    for (int k = 1; k < NUM_INPUTS; k++) {
        ...
        // The vector pipe orders its own dependent operations on a5, so the
        // accumulator carries from one iteration to the next without a barrier.
        // The cross-pipe hazard still needs a flag: srcTile is a single buffer
        // reloaded every iteration, so the next TLOAD (MTE2) overwrites what
        // this TADD (V) just read, and without this write-after-read edge MTE2
        // runs ahead and corrupts the accumulation.
